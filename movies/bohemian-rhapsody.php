...
$row = $db->get_movie(1); //Αποθήκευση του αποτελέσματος της function get_movie της κλάσης Db στη μεταβλητή $row
...
<div class="container"> <!-- Div για το περιεχόμενο της σελίδας -->
  <div class="content-container"> <!-- Container div για τις πληροφορίες της ταινίας -->
    <?php echo '<div class="movie-page-content" style="background: linear-gradient(to bottom, rgba(0,0,0,0), #151515), url('.$row['movieCover'].') no-repeat bottom fixed; background-size: cover;">'; 
  ...
  if(isset($_SESSION['login'])){ //Έλεγχος αν υπάρχει συνδεμένος χρήστης
    ...
    $fname = $row2['firstName'];  //Αποθήκευση πληροφοριών απο τη βάση στις ανάλογες μεταβλητές
    $lname = $row2['lastName'];
    $email = $row2['email'];
  ...
  if(isset($_POST['submit'])) //Έλεγχος αν έχει "πατηθεί" από τον χρήστη το κουμπί submit και εκτέλεση του query εισαγωγής τιμών στη βάση δεδομένων
  {
